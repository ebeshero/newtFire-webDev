    ...
    <xsl:template match="/">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
            ...
            <g transform="translate(50,550)">
                <text x="40" y="-500" style="font-family:sans-serif;font-size:25px" fill="black" font-stretch="expanded">Comment Tone Percentages Overall</text>
                <text x="40" y="-460" style="font-family:sans-serif;font-size:10px" fill="black" font-stretch="expanded">Number of Obscenities used: <xsl:value-of select="count(//obscenity)"/></text>
                <text x="40" y="-450" style="font-family:sans-serif;font-size:10px" fill="black" font-stretch="expanded">Number of Emotes used: <xsl:value-of select="count(//emote)"/></text>
                <text x="40" y="-440" style="font-family:sans-serif;font-size:10px" fill="black" font-stretch="expanded">Number of Acronyms used: <xsl:value-of select="count(//acronym)"/></text>
               <text x="40" y="-430" style="font-family:sans-serif;font-size:10px" fill="black" font-stretch="expanded">Number of Times Excessive Punctuation used: <xsl:value-of select="count(//expunc)"/></text>
               <text x="40" y="-420" style="font-family:sans-serif;font-size:10px" fill="black" font-stretch="expanded">Number of Times All Caps used: <xsl:value-of select="count(//caps)"/></text>
                ...
                <xsl:variable name="positive" select="(count(//comment[@tone='positive'])) div $comment"/>
                <xsl:variable name="negative" select="(count(//comment[@tone='negative'])) div $comment"/>
                <xsl:variable name="neutral" select="(count(//comment[@tone='neutral'])) div $comment"/>
