...
let $distinctBases := $launchBases => distinct-values() => string-join('ARRRRRGH! ')
let $launchPads := $rocketColl//launch[@sDateTime ! substring-before(., '-') = $y]/preceding-sibling::launchPad/@padNum/string()
let $distinctPads := $launchPads => distinct-values() => string-join('ARRRRRRGH! ')
order by $y
return 
    (:concat($y, 'Launch Base: ', $distinctBases, 'Launch Pad: ', $distinctPads):)
