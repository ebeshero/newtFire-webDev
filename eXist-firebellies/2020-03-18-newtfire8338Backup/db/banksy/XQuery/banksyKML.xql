...
let $names := $banksy//bibl/title[following-sibling::location]/string()[string-length() gt 0] 
for $n in $names
let $tree := $banksy/element()
let $description := $tree[descendant::bibl/title[. = $n]]/following::body/desc/string() ! normalize-space()
