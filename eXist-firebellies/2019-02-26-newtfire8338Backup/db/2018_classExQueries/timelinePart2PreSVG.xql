...
(: Checking these variables:  :($minHaydon, $maxHaydon) :)
declare variable $maxHaydonYear := xs:integer(tokenize($maxHaydon, '-')[1]);
declare variable $svgMinHaydonYr := ($minHaydonYear - $minYear) * 365;
declare variable $svgMaxHaydonYr := ($maxHaydonYear - $minYear) * 365;
(:Checking these variables:  :($svgMinHaydonYr, $svgMaxHaydonYr) :)
declare variable $minHaydonDayAdd := xs:integer(format-date($minHaydon, '[d]'));
(:Notice: Without xs:integer this outputs as a string in quotation marks. We need to convert it into an integer  :)
declare variable $svgMinHaydonDate := $svgMinHaydonYr + $minHaydonDayAdd;
$svgMinHaydonDate
...
return $doc-db-uri
(: Output at http://newtfire.org:8338/exist/rest/db/jonhoranic/ClassExTimeLineP1.svg :)
:)
