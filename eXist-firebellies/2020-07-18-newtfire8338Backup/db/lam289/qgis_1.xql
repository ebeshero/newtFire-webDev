...
let $locationMatch := $location[preceding::typing/@type[contains(., $d)]]/string()
let $distLM := $locationMatch => distinct-values()

for $l in $distLM
let $lat := $qgisPoke//locations/landmark[preceding::typing/@type[contains(., $d)]][@n[contains(., $l)]]/@lat => distinct-values()
let $lon := $qgisPoke//locations/landmark[preceding::typing/@type[contains(., $d)]][@n[contains(., $l)]]/@lon => distinct-values()
