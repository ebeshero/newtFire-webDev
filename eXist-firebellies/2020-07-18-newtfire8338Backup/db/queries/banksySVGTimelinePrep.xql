...
declare variable $totDate := $date => count();
(: returns 106 total dates. Note there are 59 distinct values of dates. :)
declare variable $hyphenDate := $date[contains(., '-')];
declare variable $countHyphDate := $hyphenDate => count();
(: returns 44 hyphenated dates :)
...
<svg:svg width="2000" height="3000">
   <svg:g transform="translate(30, 30)">
          <svg:line x1="0" y1="0" x2="0" y2="{($maxYear - $minYear) * $timelineSpacer}" style="stroke:blue;stroke-width:2;"/>  
        {
         ...
         return
          <svg:g>     
            <svg:circle cx="0" cy="{$i * $timelineSpacer}" r="{$countYearTitles * 2}" stroke="purple" stroke-width="3" fill="violet" />
            <svg:text x="10" y="{$i * $timelineSpacer}" style="fill:navy; stroke-width: 2; text-anchor:start">{$year}</text>
             
            <svg:text x="50" y="{$i * $timelineSpacer}" style="fill:navy; stroke-width: 2; text-anchor:start">{string-join($thisYearTitles, ', ')}</text>
        <svg:rect x="75" y="{$i * $timelineSpacer + 20}" height="20" width="{$countSprayPaint * 10}"
        style="fill: aqua"/>
        <svg:text x="78" y="{$i * $timelineSpacer + 35}" style="fill:navy; stroke-width: 2; text-anchor:start"> {$CSPlabel}</text>  
        <svg:rect x="{75 + $countSprayPaint * 10}" y="{$i * $timelineSpacer + 20}" height="20" width="{$countCanvas * 10}"
        style="fill: lavender"/>
        <svg:text x="{78 + $countSprayPaint * 10}" y="{$i * $timelineSpacer + 35}" style="fill:navy; stroke-width: 2; text-anchor:start"> {$CClabel}</text>  
